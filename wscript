...
def build(bld):
	...
	bld(
		rule = '${PYTHON} ${IMG_TO_SRC} -o ${TGT[0]} ${SRC} ' + \
			'-f RGB333',
		source = 'images/tanks.png',
		target = ['tanks_rgb333.c', 'tanks_rgb333.h']
	)
	bld(
		rule = '${PYTHON} ${IMG_TO_SRC} -o ${TGT[0]} ${SRC} ' + \
			'-f RGB333',
		source = 'images/tank.png',
		target = ['tank_rgb333.c', 'tank_rgb333.h']
	)
	bld(
		rule = '${PYTHON} ${IMG_TO_SRC} -o ${TGT[0]} ${SRC} ' + \
			'-f RGB333',
		source = 'images/bullet.png',
		target = ['bullet_rgb333.c', 'bullet_rgb333.h']
	)
	bld.program(
		features = 'cxx',
	...
	bld.program(
		features = 'cxx',
		source = ['sprite_anim.c', 'sprites_rgb333.c', 'tanks_rgb333.c', 'tank_rgb333.c', 'bullet_rgb333.c'],
		includes = ['build/'],
		use = 'emulator',
		target = 'sprite_anim'
	)
